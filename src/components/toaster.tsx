  ...
  update(id: string, updates: Partial<Omit<Toast, "timeout">>) {
    ...
    if (toast) {
      ...
      if (updates.type && updates.type !== "loading") {
        toast.dismiss = new Date().getTime() + toastFadeoutTimeout;
      }
      this.notifyListeners();
    }
  }
...
export function ToastComponent(toast: ToastInternals) {
  return (
    <motion.div
      ...
      className="border-border bg-secondary/30 text-foreground flex w-fit items-center rounded-full border p-1.5 pr-3 backdrop-blur-lg"
    >
      {toast.type === "success" && (
        <CheckCircle2 className="size-5 text-green-500" />
      )}
      {toast.type === "error" && <XCircle className="size-5 text-red-500" />}
      {toast.type === "info" && <InfoIcon className="size-5 text-blue-500" />}
      {toast.type === "loading" && (
      ...
      <motion.div
        ...
        transition={{ type: "spring", bounce: 0.2, duration: 0.3, delay: 0.05 }}
